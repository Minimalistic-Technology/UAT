...
export const updateProfile = async (req, res, next) => {
    try {
        const { firstName, lastName, phone, skills, languages, experience, education, location, } = req.body;
...
export const uploadAvatar = async (req, res, next) => {
    try {
        ...
        const result = await uploadToCloudinary(req.file.buffer, "job_portal/avatars", "image", `avatar-${req.user.id}-${Date.now()}`);
        ...
        res
            .status(200)
            .json(new ApiResponse(200, { avatarUrl: updatedUser?.avatar?.url }, "Avatar uploaded successfully"));
    }
    catch (error) {
        next(error);
    }
...
export const uploadResume = async (req, res, next) => {
    try {
        ...
        if (req.user.resume?.publicId) {
            ...
            await deleteFromCloudinary(req.user.resume?.publicId, isRaw ? "raw" : "image");
        }
        const result = await uploadToCloudinary(req.file.buffer, "job_portal/resumes", "raw", `resume-${req.user.id}-${Date.now()}`, "pdf");
        const updatedUser = await User.findByIdAndUpdate(req.user.id, {
            resume: {
                url: result.secure_url,
                publicId: result.public_id,
...
export const submitKyc = async (req, res, next) => {
    try {
        ...
        if (!files?.photo?.[0] || !files?.lightbill?.[0]) {
            throw new ApiError(400, "Both photo and lightbill documents are required.");
        }
        ...
        const photoResult = await uploadToCloudinary(files.photo[0].buffer, "job_portal/kyc_photos", "image", `kyc-photo-${req.user.id}-${Date.now()}`);
        const isLightbillPdf = files.lightbill[0].mimetype === "application/pdf";
        const lightbillResult = await uploadToCloudinary(files.lightbill[0].buffer, "job_portal/kyc_lightbills", isLightbillPdf ? "raw" : "image", `kyc-lightbill-${req.user.id}-${Date.now()}`);
        const kycData = {
            user: req.user.id,
            companyName,
            aadharNo,
        ...
        if (!kyc) {
            ...
            throw new ApiError(500, "Failed to submit KYC details. Please try again later.");
        }
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, responseToSend, "KYC Details Submitted Successfully!"));
    }
    catch (error) {
        next(error);
    }
