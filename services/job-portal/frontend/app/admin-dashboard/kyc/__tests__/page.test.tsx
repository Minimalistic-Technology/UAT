...
jest.mock("@/features/admin/components/kyc-application-table", () => {
  return {
    KycTable: function MockKycTable({ applications, isLoading, isUpdating, onUpdateStatus }: any) {
      return (
        <div data-testid="kyc-table">
          {isLoading && <div data-testid="kyc-loading">Loading applications...</div>}
          {!isLoading && applications.length === 0 && <div data-testid="kyc-empty">No applications found.</div>}
          {!isLoading && applications.map((app: any) => (
            <div key={app._id} data-testid="kyc-row">
              ...
              <button 
                ...
                onClick={() => onUpdateStatus(app._id, "rejected", "Invalid docs")}
                disabled={isUpdating}
              >
                Reject
              </button>
...
describe("Admin KYC Management Page", () => {
  ...
  it("renders error state when fetch fails", () => {
    ...
    expect(screen.getByText("Failed to load KYC applications.")).toBeInTheDocument();
  ...
  it("renders KYC applications when fetch succeeds", () => {
    (useGetKycApplications as jest.Mock).mockReturnValue({
      data: {
        data: {
          applications: [
            { _id: "1", companyName: "Tech Corp", status: "pending", user: { email: "tech@corp.com" } },
            { _id: "2", companyName: "Design LLC", status: "approved", user: { email: "design@llc.com" } },
          ],
  ...
  it("changes status filter and resets page to 1", async () => {
    (useGetKycApplications as jest.Mock).mockReturnValue({
      data: {
        data: {
          applications: [],
          pagination: { currentPage: 2, totalPages: 5, hasNextPage: true, hasPrevPage: true },
        },
      },
      isLoading: false,
      isError: false,
    ...
    expect(useGetKycApplications).toHaveBeenCalledWith({ page: 1, limit: 10, status: undefined });
    ...
    expect(useGetKycApplications).toHaveBeenCalledWith({ page: 1, limit: 10, status: "pending" });
  });
  ...
  it("navigates between pages using pagination buttons", () => {
    (useGetKycApplications as jest.Mock).mockReturnValue({
      data: {
        data: {
          applications: [{ _id: "1", companyName: "Tech Corp", status: "pending" }],
    ...
    expect(useGetKycApplications).toHaveBeenCalledWith({ page: 2, limit: 10, status: undefined });

    // Mock page 2 response
    (useGetKycApplications as jest.Mock).mockReturnValue({
      data: {
        data: {
          applications: [{ _id: "2", companyName: "Design LLC", status: "approved" }],
  ...
  it("calls updateStatus when approve/reject actions are triggered", () => {
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith({ applicationId: "1", status: "approved", note: undefined });

    // Reject
    const rejectBtn = screen.getByTestId("reject-1");
    fireEvent.click(rejectBtn);
    expect(mockUpdateStatus).toHaveBeenCalledWith({ applicationId: "1", status: "rejected", note: "Invalid docs" });
  });
