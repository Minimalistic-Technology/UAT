...
import { PlanSkeleton } from "@/skeletons/employer/plans";
import { EmptyState } from "@/errors/employer/plans";
import {
  Table,
  TableBody,
  TableCell,
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      <div className="px-6 lg:px-10">
        ...
        >
          {isLoadingState ? (
            Array.from({ length: skeletonCount }).map((_, i) => <PlanSkeleton key={i} />)
        ...
        {!isLoadingState && sortedPlans.length > 0 && (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-slate-200 dark:border-slate-800">
                    <TableHead className="w-1/3 px-4 py-5 text-xs font-bold tracking-widest text-slate-500 uppercase">
                      Features
                    </TableHead>
                    {sortedPlans.map((plan) => (
                      <TableHead
                ...
                <TableBody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  <TableRow className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    <TableCell className="px-4 py-5 font-semibold text-slate-700 dark:text-slate-300">
                      Job Postings
                    </TableCell>
                    {sortedPlans.map((plan) => (
                      <TableCell
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div className="rounded-[20px] border-0 bg-white p-6 shadow-[0_2px_15px_rgba(0,0,0,0.04)] dark:bg-slate-900">
          ...
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            ...
            dashboard settings. If you upgrade, the change is immediate.
          </p>
        </div>
        <div className="rounded-[20px] border-0 bg-white p-6 shadow-[0_2px_15px_rgba(0,0,0,0.04)] dark:bg-slate-900">
          <h4 className="mb-2 font-bold text-[#2563eb]">
          ...
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            ...
            requirements, scoring them on skills to surface top talent faster.
          </p>
        </div>
        <div className="rounded-[20px] border-0 bg-white p-6 shadow-[0_2px_15px_rgba(0,0,0,0.04)] dark:bg-slate-900">
          <h4 className="mb-2 font-bold text-[#2563eb]">
          ...
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            Absolutely. Select plans come with a 14-day free trial. No credit
            card is required to start exploring the premium recruitment
            features.
          </p>
        </div>
        <div className="rounded-[20px] border-0 bg-white p-6 shadow-[0_2px_15px_rgba(0,0,0,0.04)] dark:bg-slate-900">
          ...
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            ...
            priority chat support. Enterprise clients have a dedicated success
            manager.
          </p>
        </div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-[2rem] bg-[#0b51da] p-10 text-center shadow-2xl md:p-14">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="mb-10 max-w-xl text-center text-sm font-medium text-blue-100 md:text-base">
            Join top companies using our Job Portal to build world-class teams
