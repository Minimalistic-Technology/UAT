  ...
  const filters = useMemo(
    () => ({
      ...
      maxStipend: searchParams.get("maxStipend")
        ? Number(searchParams.get("maxStipend"))
        : undefined,
      stipendType: searchParams.get("stipendType") || "",
      workMode: searchParams.get("workMode")?.split(",").filter(Boolean) || [],
      roleCategory:
        searchParams.get("roleCategory")?.split(",").filter(Boolean) || [],
      companyType:
        searchParams.get("companyType")?.split(",").filter(Boolean) || [],
      durationMonths:
        searchParams.get("durationMonths")?.split(",").filter(Boolean) || [],
      salaryRanges:
        searchParams.get("salaryRanges")?.split(",").filter(Boolean) || [],
      stipendRanges:
        searchParams.get("stipendRanges")?.split(",").filter(Boolean) || [],
      experienceRanges:
