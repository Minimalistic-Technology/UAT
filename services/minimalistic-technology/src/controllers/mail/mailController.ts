...
export const sendMail = asyncHandler(async (req: Request, res: Response) => {
  ...
  const safeName = String(formData.name || "there").replace(/[<>]/g, "");
  const safeCompany = String(formData.company || "").replace(/[<>]/g, "");
  const safePhone = String(formData.phone || "").replace(/[<>]/g, "");

  const receiverEmail =
    req.body.to ||
    formData.to ||
    process.env.RECEIVER_EMAIL ||
    "info@minimalistic-technology.com";

  const fromEmail =
    process.env.FROM_EMAIL || "no-reply@minimalistic-technology.com";

  const msg: any = {
    to: receiverEmail,
    from: fromEmail,
    replyTo: userEmail, // Replies to the notification go to the user
    dynamicTemplateData: {
      ...formData,
      templateName,
      subject:
        formData.subject ||
        `New Inquiry: ${templateName || "Minimalistic Technology"}`,
    },
  };
  ...
  if (templateId) {
    ...
    msg.subject =
      formData.subject || "New Inquiry from Minimalistic Technology";
  ...
  try {
    ...
    if (formData.email) {
      try {
        console.log(
          `Process 2: Sending User Confirmation to ${formData.email}...`,
        );
        const userMsg = {
          to: userEmail,
          from:fromEmail,
                            ...
                            <div style="padding: 30px; line-height: 1.6;">
                                ...
                                <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
                                    ...
                                    <ul style="list-style: none; padding: 0;">
                                        ...
                                        ${formData.phone ? `<li><strong>Phone:</strong> ${safePhone}</li>` : ""}
                                        ${formData.company ? `<li><strong>Company:</strong> ${safeCompany}</li>` : ""}
                                    </ul>
                                </div>
